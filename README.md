...
これはセキュリティ監査済み製品ではありません。現段階は、設計を検証しながら実用化するための`v0.6.3`です。
...
ダブルクリック展開用shellの警告・失敗MessageBoxは、日英の状況説明と技術詳細を併記します。
...
- `v0.6.3`はWindows x64とnative ARM64を別assetで配布します。ARM64の実測範囲と未検証device境界は[ARM64対応状況](docs/ARM64.md)にあります。
