...
さらに、Job Objectで子プロセス数を1、メモリ上限を設定し、指定時間を超えた処理を終了します。展開完了後は、一時領域から直接利用せず、検査済みの通常ファイルだけを新しいフォルダへコピーしてからrenameします。作成時も圧縮元を監査・複製してからAppContainer内で処理し、生成書庫を別sandboxへ再展開してtree fingerprintが一致するまで公開しません。
