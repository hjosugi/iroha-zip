...
これはセキュリティ監査済み製品ではありません。現段階は、設計を検証しながら実用化するための`v0.5.1`です。
...
- `v0.5.1`はWindows x64とnative ARM64を別assetで配布します。ARM64の実測範囲と未検証device境界は[ARM64対応状況](docs/ARM64.md)にあります。
