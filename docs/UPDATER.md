...
iroha-zipはupdaterを出荷しておらず、backgroundでnetworkへ接続しません。`v0.5.2`を含む公式版は
...
`v0.5.2`, are unsigned, so a digest or GitHub artifact attestation alone cannot authorize
self-update. OPS-001
