...
iroha-zipはupdaterを出荷しておらず、backgroundでnetworkへ接続しません。`v0.6.2`を含む公式版は
未署名なので、digestやGitHub release／workflow attestationsだけを根拠に自己更新を有効化しません。
...
`v0.6.2`, are unsigned, so neither a digest nor GitHub release/workflow attestations can authorize
self-update. OPS-001
