...
- Audited regular-object copy into a unique external creation tree; exact source-tree fingerprint comparison; bounded recursive per-object Package-SID read/execute-only sealing before relative `.` creation; created-archive full-root re-extraction in a second sandbox; handle-retained create-new publication; deterministic root/mutation/mismatch/budget tests; and a local libarchive 3.8.9 ZIP/7z/TAR/TAR.GZ/empty/Japanese-name round trip through the production path
- Windows sandbox-copy backend preparation with a fixed `asInvoker`/long-path/UTF-8 process manifest, fail-closed Win32 resource update, and byte-for-byte resource readback to avoid libarchive 3.8.6+ Unicode-name regression without changing the imported backend
