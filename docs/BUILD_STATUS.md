...
The current Linux suite contains 70 passing default-feature tests, plus the feature-gated minimized fuzz-regression gate. Windows CI additionally runs the source-handle sharing test that verifies open snapshots block writes and renames.
...
A separate fixed-label `windows-2022` / `windows-2025` x64 matrix builds release binaries, exports the verified backend, and produces machine-readable archive/isolation and settings artifacts. It is designed to exercise token/capability inspection, loopback denial, timeout, memory limit, explicit cleanup, four create/read formats, Japanese and long paths, invalid input, shell invocation, settings save, and settings diagnosis. These new workflow steps have been parsed locally but have not yet produced a GitHub Actions result for the current local branch. The Server matrix is not Windows 10/11 desktop evidence; see [the exact contract](WINDOWS_E2E.md).
