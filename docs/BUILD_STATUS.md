...
The current Linux suite contains 88 passing default-feature tests, plus the feature-gated minimized fuzz-regression gate (89 with all features). Windows CI additionally runs source-file and directory-handle sharing tests that verify open snapshots block writes and renames.
