...
3. `gh release verify v0.6.3 --repo hjosugi/iroha-zip`で、tagと11 assetを固定するGitHub release attestationを確認する。
4. `gh release verify-asset v0.6.3 <file> --repo hjosugi/iroha-zip`で、downloadしたfileがそのReleaseに含まれることを確認する。
...
```powershell
Get-FileHash .\iroha-zip-0.6.3-windows-x64.zip -Algorithm SHA256
# Windows on ARMの場合 / For Windows on ARM:
Get-FileHash .\iroha-zip-0.6.3-windows-arm64.zip -Algorithm SHA256
```
...
3. Verify the GitHub release attestation binding the tag and all 11 assets with `gh release verify v0.6.3 --repo hjosugi/iroha-zip`.
4. Confirm that the downloaded file belongs to that Release with `gh release verify-asset v0.6.3 <file> --repo hjosugi/iroha-zip`.
