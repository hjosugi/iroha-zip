...
Windowsの子processはJob Objectへ割り当てた`CREATE_SUSPENDED`状態で生成します。親が要求modeの`TokenIsAppContainer`、LPAC指定時の`TokenIsLessPrivilegedAppContainer`、capability 0を確認した後だけprimary threadを1回resumeします。属性設定、process生成、token照合、resumeのいずれかが失敗した場合は、子がbackend codeを実行する前にJobごと終了し、通常AppContainerとして再試行しません。OS build番号による推測ではなくruntime検査でfail closedにします。`--allow-unsandboxed`は利用者がコマンドごとに明示した場合だけ存在する別の危険な診断経路です。
...
作成backendへ通常の圧縮元pathは渡しません。信頼する親プロセスが、通常file／directoryだけを一意な外部staging treeへ監査付きでcopyし、path深さ・長さ、file数、directory数、単一file size、合計sizeとtree fingerprintを元sourceに対して再検査します。全entryをDACL封印した後、親が決定的で有界なPAX streamへ直列化します。各fileは保持中の監査handleからcopyされ、PAX自体もidentity・長さ・SHA-256付きhandleでbackend実行中まで固定します。backend processには通常source、staging tree operand、絶対pathを渡さず、sandbox rootをcurrent directoryとして固定`@source.pax.tar`だけを渡します。これにより、Windows版libarchiveのdisk readerがAppContainerから許可されないvolume rootへ`GetDiskFreeSpaceW`を行う経路を使用しません。[libarchive 3.8.9の7z writer](https://github.com/libarchive/libarchive/blob/v3.8.9/libarchive/archive_write_set_format_7zip.c#L658-L678)が呼ぶ[Windows temp実装](https://github.com/libarchive/libarchive/blob/v3.8.9/libarchive/archive_util.c#L343-L387)の`GENERIC_READ | GENERIC_WRITE | DELETE`一時fileのためにだけ、sandbox childの専用scratchへ継承可能なread／write／deleteを付与します。[WindowsのAppContainer起動規則](https://learn.microsoft.com/en-us/windows/win32/secauthz/implementing-an-appcontainer)がhost側LocalAppDataをprofile内へ再ルートするため、環境の`LOCALAPPDATA`、`USERPROFILE`、`TEMP`、`TMP`にはすべてhost側pathを渡し、scratchは対応する固定`AC\Temp`に作成します。すでにprofile内へ写像済みの値は二重変換されるため拒否します。child自身の`GetTempPathW`結果と専用scratchのidentityが一致し、libarchiveと同じCNG乱数・read／write／delete・delete-on-close probeが成功することをE2Eで必須検証します。scratchはJob／tree monitorの対象であり、使用前後に空であることを確認して親が削除します。残留object、上限超過、sandbox外scratch指定はいずれもfail closedです。以前の日本語名での`@archive` access violationは、sandbox EXEへ固定UTF-8 process manifestを適用する現在の互換境界で回避します。作成終了後はstaging treeとPAX streamを再照合し、生成書庫をhandleから別sandboxへ渡します。libarchiveが付ける単一の`./` prefix、Windows native表記の単一`.\` prefix、または同じroot markerだけを作成物専用のraw listing policyで正規化し、二重prefix、その他のbackslash形式、親参照、絶対pathなどは拒否します。外部書庫のpolicyはこの例外を持ちません。再展開した完全rootがsourceと一致し、さらに書庫identity・時刻・長さ・SHA-256が検証時と一致するhandleからだけ最終出力へcopyします。内容不一致、同一サイズ改変、identity置換、危険なlistingはいずれも出力前にfail closedになります。
...
単体のGZ／BZ2／XZ／Zstandard／compress streamはraw形式なので、`archive_read_support_format_all`を使う通常書庫経路へ暗黙に混ぜません。外側の拡張子が期待filterを固定し、拡張子を1つ外してWindows policyを通した1 componentだけを出力名にします。stream内の元filenameやpath metadataは信頼しません。専用子はmanifest固定DLLからraw formatだけを有効化し、実filter codeとraw formatを照合して、事前passでも全dataを読み、libarchiveが返した形式／復号errorと単一file上限を検出します。入力は固定`input/archive.bin`、実展開先は新規`output`だけに限定し、create-new fileへ64 KiBずつ書きます。別の新規AppContainerで同じ処理を再実行し、通常のpost-extraction audit／fingerprint／atomic publicationへ合流します。filter不一致、libarchive報告error、timeout、Job/memory/process/容量上限、追加entry、close/free、書込、監査のどれかが失敗するとpartial fileを削除し、最終destinationを公開しません。圧縮形式自体を暗号学的な内容真正性の根拠にはせず、成功した出力も未信頼fileとして扱います。`tar.gz`等の複合拡張子はraw分岐から明示除外します。
...
- 最初の実環境Authenticode署名済みrelease証跡と独立レビュー、および署名済みアップデート（未署名`v0.6.2`は[release run 31877964740](https://github.com/hjosugi/iroha-zip/actions/runs/31877964740)でGitHub immutable policy、draft-first検証、公開後のexact 11 asset readback、hosted-runner限定attestationを完了し、[恒久metadata snapshot](https://github.com/hjosugi/iroha-zip/tree/main/evidence/releases/v0.6.2)も保存した。これはpublisher署名の代替ではない）
