...
| Create input | The trusted parent copies only audited regular objects into a unique external staging tree, compares the complete source fingerprint, and protects every file/directory DACL individually. It serializes that sealed tree into a bounded PAX stream held by identity/length/SHA-256; the backend receives only fixed sandbox-local `@source.pax.tar`, never the normal source path or a filesystem-tree operand. |
