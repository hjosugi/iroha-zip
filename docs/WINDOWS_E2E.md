...
| Create input | The trusted parent converts the audited staging tree into a deterministic, entry/size/path-bounded PAX stream. The backend receives only that stream, and both the stream fingerprint and staging-tree fingerprint must remain unchanged. |
