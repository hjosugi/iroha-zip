...
| Encrypted ZIP | The verified backend generates controlled ZipCrypto, AES-128, and AES-256 ZIPs. UI Automation requires the exact bilingual native dialog, an enabled/focusable protected edit, and accessible confirm/cancel buttons. A sealed internal child receives one value through an explicitly inherited anonymous-pipe read handle only after AppContainer verification, then loads the manifest-pinned libarchive DLL and reproduces the complete SHA-256 tree. A Japanese public sentinel must be absent from stdout/stderr. Wrong-password exit and explicit cancellation must publish no destination; cancellation must produce no output. |
