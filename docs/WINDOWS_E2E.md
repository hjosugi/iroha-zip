...
| Encrypted ZIP | The verified backend generates controlled ZipCrypto, AES-128, and AES-256 ZIPs. UI Automation requires the exact bilingual native dialog, an enabled/focusable protected edit, and standard `InvokePattern` exposure on confirm/cancel. Because Microsoft documents that [`SetWindowTextW`](https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-setwindowtextw) cannot change another process's control, the harness sets only its public fixture value through a bounded synchronous `WM_SETTEXT` and requires the edit-control result. It locates the native button through the UIA tree, delivers its bounded synchronous `WM_COMMAND` / `BN_CLICKED` notification to the real dialog procedure, and requires process-identity-checked native-window closure within ten seconds; checking only `IsWindow` would mistake a recycled HWND for the original dialog. This avoids Microsoft-documented [`BM_CLICK`](https://learn.microsoft.com/en-us/windows/win32/controls/bm-click) failure when a dialog is not active, because a non-interactive hosted desktop cannot guarantee activation. A sealed internal child receives one value through an explicitly inherited anonymous-pipe read handle only after AppContainer verification, then loads the manifest-pinned libarchive DLL and reproduces the complete SHA-256 tree. The public ASCII fixture password must be absent from stdout/stderr; a separate native probe covers Japanese input. Wrong-password exit and explicit cancellation must publish no destination; cancellation must produce no output. |
| Cleanup | All seven probe profiles, their profile roots, and their owned external staged-source roots are absent after explicit cleanup. Profile deletion follows Microsoft's indeterminate-failure recovery contract with at most 20 attempts, 50 milliseconds apart; exhaustion remains a fatal cleanup failure. |
...
Normal create, preview, extract, shell, and doctor success now call explicit sandbox cleanup. Backend launch, timeout, resource-monitor, and nonzero-exit failures also attempt cleanup before returning the original failure; a cleanup failure is combined into the returned error instead of being hidden by `Drop`. Microsoft documents that a failed [`DeleteAppContainerProfile`](https://learn.microsoft.com/en-us/windows/win32/api/userenv/nf-userenv-deleteappcontainerprofile) leaves profile state undetermined and must be called again. iroha-zip therefore performs a sub-second bounded retry only after its process and file handles have closed, and still fails closed if every attempt fails.
