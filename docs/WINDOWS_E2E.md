...
| Create input | The trusted parent converts the audited staging tree into a deterministic, entry/size/path-bounded PAX archive, opens it without write/delete sharing, and removes the original staging copy before child launch. One monitored AppContainer pass materializes that PAX and must reproduce the exact source fingerprint. The parent then removes and recopies the verified backend, deletes the PAX, seals the materialized tree read-only to the Package SID, and lets the fresh backend archive only relative `.`. |
...
| Create/read | ZIP, 7z, TAR, and TAR.GZ are created from the fingerprint-matched, DACL-sealed materialized PAX tree, internally re-extracted in a second AppContainer before publication, then independently previewed, extracted, and compared by relative path, type, length, and SHA-256 by the harness. |
