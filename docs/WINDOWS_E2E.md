...
| Create input | The trusted parent converts the audited staging tree into a deterministic, entry/size/path-bounded PAX stream, opens it without write/delete sharing, and passes the audited handle as inherited stdin. The backend receives `@-`, not a source or PAX path; both the stream fingerprint and staging-tree fingerprint must remain unchanged. |
