...
| Create input | The trusted parent copies only audited regular objects into a unique external staging tree, compares the complete source fingerprint, and protects every file/directory DACL individually. The Package SID receives read/execute only; the backend receives that tree as its working directory and only relative `.`, never the normal source path. |
| UTF-8 backend | After exact source and sandbox-copy hashes pass, the temporary backend EXE receives a fixed `asInvoker`/long-path/UTF-8 process manifest. The resource is read back byte-for-byte before launch. Japanese names in all create/read paths must survive on the default English runner locale. |
...
| Create/read | ZIP, 7z, TAR, and TAR.GZ are created from the fingerprint-matched, recursively DACL-sealed staging tree, internally re-extracted in a second AppContainer before publication, then independently previewed, extracted, and compared by relative path, type, length, and SHA-256 by the harness. |
