...
This document defines the automated SAFE-001 evidence contract. The workflow and harness have run on both fixed-label GitHub runners. The latest archive matrix completes ZIP but stops at a reproducible libarchive 7z temporary-file creation error; the next run adds an exact in-container `GetTempPathW`, CNG random-number, and delete-on-close file probe. A failing diagnostic artifact is not a passing Windows result.
