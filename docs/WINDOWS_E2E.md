...
| Create input | The trusted parent converts the audited staging tree into a deterministic, entry/size/path-bounded PAX archive, opens it without write/delete sharing, and retains the audited handle through child exit. The backend receives only the fixed sandbox-local `@source.pax.tar` name, never the source-tree path; both the PAX fingerprint and staging-tree fingerprint must remain unchanged. |
...
| Create/read | ZIP, 7z, TAR, and TAR.GZ are created from the bounded PAX stream, internally re-extracted in a second AppContainer before publication, then independently previewed, extracted, and compared by relative path, type, length, and SHA-256 by the harness. |
