...
Current extension (2026-08-16): schema-v5 passed on both fixed Windows Server x64 images and native
Windows 11 ARM from exact `main` commit `71f7b674745bc8446142f4f7dbf71534839ac9fa` in
[Actions run 31891960603](https://github.com/hjosugi/iroha-zip/actions/runs/31891960603). Exact-name local
...
corpus, Settings diagnosis, capability 0, and complete cleanup. The two Server reports also record
real `SendInput` for exact forward/reverse Tab traversal, Enter save/dismissal, and Escape
close-request/cancellation. All four schema-v3 Settings reports record one completed production
backend Browse picker and two safely cancelled import pickers. The hosted ARM64 reports preserve
the same order and actions while
explicitly marking their limited non-key fallback. Disposable Windows 10/11 x64 desktop
...
of 90 days. A canonical copy of all 11 reports, each original/canonical file hash, and every source
artifact API digest is checked into the
[latest durable run snapshot](https://github.com/hjosugi/iroha-zip/tree/main/evidence/windows/31891960603)
with an ordinary Rust regression for inventory and security assertions. The earlier reviewed run is
retained beside it rather than overwritten. Periodic promotion of later reviewed runs remains open;
long-term evidence no longer depends only on expiring Actions artifacts.
...
Progress (2026-08-15): the active workflow intentionally publishes unsigned Windows x64 and native ARM64 binaries with exact tag/version/current-`main` gates, two backend-free ZIPs, six individually downloadable PE executables, SHA-256 inventories, and GitHub workflow attestations. Publication creates an explicit draft, verifies all 11 remote assets by exact name/length/SHA-256 before publishing, and requires immutable/latest readback afterward. Future publication runs also allow only bounded propagation time before requiring the GitHub release attestation and `gh release verify-asset` success for every local asset; deterministic cross-platform PowerShell tests cover delayed availability, all-asset success, asset mismatch, exhaustion, incomplete inventory, and duplicate names. The unsigned [v0.6.3 release](https://github.com/hjosugi/iroha-zip/releases/tag/v0.6.3) passed the preceding complete publication path in [Actions run 31887844084](https://github.com/hjosugi/iroha-zip/actions/runs/31887844084), after [dry run 31887126357](https://github.com/hjosugi/iroha-zip/actions/runs/31887126357) passed without publication; [tag CI run 31887844038](https://github.com/hjosugi/iroha-zip/actions/runs/31887844038) also passed on the exact source commit. All public assets were independently downloaded; API digests and byte lengths, eight checksum subjects, two sidecars, x64/ARM64 PE identities inside and outside both ZIPs, empty Authenticode Certificate Tables, bilingual package content and source-document equality, backend non-inclusion, annotated tag object `c99000335f2b64df815f029ff1d7b7d25e31a2c0`, exact commit `c3984c578f4a49f8bea37e0d22df33b7d0483621`, and nine hosted-runner-only tag-ref workflow attestations all matched. The separate GitHub release attestation bound that tag object to all 11 assets, and every downloaded asset passed `gh release verify-asset`. The fixed metadata, asset hashes, expiring workflow-artifact digests, and independent results are preserved in the [v0.6.3 release snapshot](https://github.com/hjosugi/iroha-zip/tree/main/evidence/releases/v0.6.3) and enforced by an ordinary Rust test. The package and website disclose the absence of Authenticode and provide Japanese/English verification guidance. The local build/sign/package split and strict WinVerifyTrust publisher/EKU/timestamp verifier remain available for a future owner-configured signing identity. Authenticode custody, the first signed run, and independent review remain open; no current release is claimed signed. See [the release contract](RELEASE_VERIFICATION.md).
...
[Actions run 31891960603](https://github.com/hjosugi/iroha-zip/actions/runs/31891960603) exercised the
...
Progress (2026-08-16): the settings executable declares Per-Monitor V2 awareness with Per-Monitor and legacy fallbacks. Its `WM_DPICHANGED` handler applies the suggested window rectangle, rebuilds every child layout from the 96-DPI baseline, rescales scroll state, recreates the DPI-specific system font, and recalculates both scrollbars. Windows UI Automation requires the effective PMv2 HWND context and a drift-free synthetic 96→144→96 relayout in addition to Japanese and English names, types, focus, bounds, long/non-ASCII input, dirty state, and confirmations for all 26 controls. A cross-language contract now fixes the native creation/Tab order; the real Windows test uses `SendInput` to require exact forward Tab and reverse Shift+Tab cycles, both wrap boundaries, the expected process/AutomationId after every chord, and full focus visibility after automatic scrolling. In the same process it requires Enter save, Enter saved-message dismissal, Escape dirty-close request, cancelled confirmation, process survival, and the exact persisted value. Exact-main [run 31891960603](https://github.com/hjosugi/iroha-zip/actions/runs/31891960603) preserved real-key success on Server 2022/2025 and the honest non-key hosted-ARM64 fallback in the latest durable snapshot. The schema-v3 harness completes the side-effect-free backend Browse picker through exact accessible selection of a disposable long Japanese folder, requires the selected path to reach Settings, and separately cancels both import pickers. It also verifies both Restore Defaults decisions, drives both unsaved-change Cancel decisions, and uses accessible dialog patterns across desktop/Server UIA providers. Configuration replacement and initial creation are serialized by a named Windows mutex across processes in the current session. Thread regressions cover simultaneous replacement and first creation; a Windows-only integration test releases two independent processes against the same non-ASCII path and rejects partial or leftover staging state. Windows unit tests also require bounded timeout failure, successful acquisition after release, and abandoned-owner recovery. Injected configuration rename failures require byte-identical restoration or preservation and reporting of the exact recovery-backup path. Backend import CI separately injects failure immediately after the prior backend is renamed to its unique backup, requires byte-identical restoration with no stage/backup residue, and completes a normal import successfully. A disposable per-user registry test registers all 18 archive types twice, checks exact commands/icons/capabilities, and unregisters while preserving unrelated values and an exact snapshot of every protected `UserChoice`. Real physical-monitor visual/screen-reader evidence, completed external default-app actions, UI-driven backend replacement, and broader external-state failure paths remain open. See [the detailed accessibility contract](SETTINGS_ACCESSIBILITY.md).
...
Acceptance met (2026-08-15): native GitHub `windows-11-arm` CI requires OS/process architecture `Arm64` and Rust host `aarch64-pc-windows-msvc`, runs all-target tests and Clippy, and requires PE machine `0xAA64` for all three application EXEs and every backend EXE/DLL. The signature-enforcing MSYS2 exporter and strict source/repository/package-pair validator support CLANGARM64 with exact package versions, signed database/archive hashes, payload byte comparison, provenance, SPDX, and license inventory. The current native run covers ZIP/7z/TAR/TAR.GZ creation, 14 exact read fixtures including standalone streams, Microsoft LZX CAB, and official libarchive RAR/RAR5/LHA/ZIPX fixtures, all three raw-stream negative cases, the 18-reject/one-control malicious corpus plus native policy fixtures, normal and standalone-raw shell dispatch, Japanese and English Settings, normal schema-v5 AppContainer, and the exact LPAC fail-closed branch. All five downloaded JSONs from [Actions run 31891960603](https://github.com/hjosugi/iroha-zip/actions/runs/31891960603) record those matrices as executed and every temporary root removed; exact release-source [tag CI run 31887844038](https://github.com/hjosugi/iroha-zip/actions/runs/31887844038) independently passed the native ARM64 job. The ARM64 Settings import path selects CLANGARM64 automatically and the exporter rejects non-ARM64 backend payloads. The immutable [v0.6.3 release](https://github.com/hjosugi/iroha-zip/releases/tag/v0.6.3), published by [Actions run 31887844084](https://github.com/hjosugi/iroha-zip/actions/runs/31887844084), separates x64 and ARM64 names, sidecars, ZIPs, and six standalone EXEs; requires `0x8664`/`0xAA64` both outside and inside each ZIP; and admits only the exact 11-asset inventory. Independent public re-download verified every API digest and byte length, checksum, PE identity, ZIP-to-standalone byte match, bilingual package content and source-document equality, backend non-inclusion, annotated tag identity, all nine hosted-runner-only tag-ref workflow attestations, the separate GitHub release attestation, and all 11 downloaded assets against that release attestation. Those facts are retained in the [release snapshot](https://github.com/hjosugi/iroha-zip/tree/main/evidence/releases/v0.6.3). Additional retail-device breadth remains ongoing compatibility work, not an unmet package acceptance condition; see [the ARM64 boundary](ARM64.md).
