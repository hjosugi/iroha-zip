...
Progress (2026-08-10): v0.3.1 keeps each input archive/source-file handle through validation, hashing, and copy; verifies identity, timestamps, length, and SHA-256; blocks write/delete sharing on Windows; uses `O_NOFOLLOW` on Unix; and compares deterministic path/type/content fingerprints before and after source-tree copy. Windows tree membership now comes from a bounded `GetFileInformationByHandleEx` enumeration on a reparse-aware, rename/delete-blocking parent directory handle. Directory volume/file identity is checked before/after enumeration and again between audit and copy, including empty-directory replacement regression coverage; child opens remain path-based and are independently handle/identity/fingerprint checked. Creation rechecks the staged source after the backend exits, snapshots the generated archive, passes those exact bytes into a second sandbox for raw-listing preflight and full-root re-extraction, compares the path/type/length/content tree fingerprint, rechecks source and archive identity, and publishes from a retained handle with create-new semantics. Before the create backend starts, Windows stages the source outside intrinsically writable Package profile storage, protects its DACL from inheritance, gives the ephemeral Package SID only inheritable read/execute access, and gives the parent only non-inheriting traversal/list access. The trusted parent then emits a deterministic PAX stream while rechecking path, count, per-file, total-byte, identity, timestamp, length, and SHA-256 constraints. The backend receives the audited PAX handle as inherited stdin and reads `@-`, not a staging-tree or PAX path; Windows keeps write/delete sharing denied for that handle. Post-run stream/source fingerprints and second-sandbox full-tree comparison make backend mutation fail closed. The isolation report's copied child probes parent/root/nested reads and all parent/source write denials and records four-profile cleanup; explicit sandbox cleanup also fails unless the external source root is removed. Eight fake-backend tests cover success, PAX-container monitor budgeting, empty input, source mutation, PAX-stream mutation, unsafe generated members, content mismatch, and the single-`archive`-directory heuristic boundary; three unit tests cover same-size file mutation, identical-byte file replacement, and same-name empty-directory replacement. A local libarchive 3.8.9 run passed inherited-stdin `@-` PAX ingestion and ZIP/7z/TAR/TAR.GZ create/re-extract comparison. Native child open relative to the parent handle, a real-Windows reparse race stress test, and first passing Windows ACL/create evidence remain open.
...
Progress (2026-08-10): the settings executable embeds System-DPI awareness, scales its logical layout at 100–300%, remains resizable, scrolls both axes, and follows keyboard focus into the viewport. All 26 setting/action controls have stable IDs and access keys; every action dispatch is exhaustively mapped; Windows CI has a native UI Automation smoke test for names, types, focus, bounds, long/non-ASCII input, dirty state, and confirmations. The script now opens and cancels all three safe folder-picker paths, verifies both Restore Defaults decisions, and drives both unsaved-change Cancel decisions. Configuration replacement and initial creation are serialized by a named Windows mutex across processes in the current session. Thread regressions cover simultaneous replacement and first creation; a Windows-only integration test releases two independent processes against the same non-ASCII path and rejects partial or leftover staging state. Windows unit tests also require bounded timeout failure, successful acquisition after release, and abandoned-owner recovery. Injected rename failures require byte-identical restoration or preservation and reporting of the exact recovery-backup path. Real visual/screen-reader/mixed-DPI evidence, first Windows results for the new automation/mutex/process tests, and actual external-state/import-rollback automation remain open. See [the detailed accessibility contract](SETTINGS_ACCESSIBILITY.md).
