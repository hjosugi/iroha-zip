...
Progress (2026-08-14): the schema-v4 fixed-label Windows Server 2022/2025 matrix passed in [Actions run 31768440143](https://github.com/hjosugi/iroha-zip/actions/runs/31768440143). Its downloaded JSON records a verified MSYS2 backend, zero-capability AppContainer identity, active-loopback denial, deterministic timeout and memory rejection, abnormal child termination, corrupt-loader process-creation rejection, one effective process-temp path, seven explicit profile/root cleanups, ZIP/7z/TAR/TAR.GZ creation-preview-re-extraction, controlled TAR.BZ2/TAR.XZ/TAR.ZST/TAR.Z reads, matching SHA-256 trees, Japanese and >260-character paths, invalid-input non-publication, shell invocation, the generated malicious corpus, and the 26-control English settings save/doctor path. A Windows regression also holds the child suspended through a forced two-second token-verification failure and requires zero child stdout. [Actions run 31770399421](https://github.com/hjosugi/iroha-zip/actions/runs/31770399421) additionally passed a Microsoft-signed `makecab.exe` LZX CAB on both Server images; downloaded reports record `Valid` Authenticode, exact generator/archive hashes, matching source/extracted tree hashes, and cleanup. Disposable Windows 10/11 x64 runs, RAR/LHA/ZIPX/raw-stream fixtures, broader LPAC denial measurements, concurrent race stress, and long-term evidence retention remain open; see [the evidence contract](WINDOWS_E2E.md).
...
Progress (2026-08-14): the active workflow intentionally publishes unsigned Windows x64 binaries with exact tag/version/current-`main` gates, a complete ZIP, three individually downloadable PE executables, SHA-256 inventories, and GitHub artifact attestations. Publication creates an explicit draft, verifies all six remote assets by exact name/length/SHA-256 before publishing, and requires immutable/latest readback afterward. The unsigned [v0.4.1 release](https://github.com/hjosugi/iroha-zip/releases/tag/v0.4.1) became the first release under the repository immutable policy in [Actions run 31769440507](https://github.com/hjosugi/iroha-zip/actions/runs/31769440507). Its six public assets, tag commit, checksums, x64 PE inventory, ZIP contents, and five tag-ref attestations were independently downloaded and verified. The older `v0.4.0` predates policy enforcement and remains mutable. The package and website disclose the absence of Authenticode and provide Japanese/English verification guidance. The local build/sign/package split and strict WinVerifyTrust publisher/EKU/timestamp verifier remain available for a future owner-configured signing identity. Authenticode custody, the first signed run, and independent review remain open; no current release is claimed signed. See [the release contract](RELEASE_VERIFICATION.md).
...
Progress (2026-08-14): native GitHub `windows-11-arm` CI requires OS/process architecture `Arm64` and Rust host `aarch64-pc-windows-msvc`, runs all-target tests and Clippy, and requires PE machine `0xAA64` for all three application EXEs and every backend EXE/DLL. The signature-enforcing MSYS2 exporter and strict source/repository/package-pair validator support CLANGARM64 with exact package versions, signed database/archive hashes, payload byte comparison, provenance, SPDX, and license inventory. The complete native run covers ZIP/7z/TAR/TAR.GZ creation, five read fixtures including Microsoft LZX CAB, the 18-reject/one-control malicious corpus plus native policy fixtures, shell, Japanese and English Settings, normal schema-v4 AppContainer, and the exact LPAC fail-closed branch. All five downloaded JSONs from [Actions run 31770908971](https://github.com/hjosugi/iroha-zip/actions/runs/31770908971) record those matrices as executed and every temporary root removed. The ARM64 Settings import path now selects CLANGARM64 automatically. The `v0.5.0` Release contract builds x64 and ARM64 separately on native runners, requires `0x8664`/`0xAA64` both outside and inside each ZIP, and admits only the architecture-separated 11-asset inventory with per-architecture attestations and exact draft/published readback. The issue remains open until the dual-architecture dry run and immutable public Release are independently downloaded and verified; see [the ARM64 boundary](ARM64.md).
