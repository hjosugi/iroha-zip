...
Progress (2026-08-10): the settings executable embeds System-DPI awareness, scales its logical layout at 100–300%, remains resizable, scrolls both axes, and follows keyboard focus into the viewport. All 26 setting/action controls have stable IDs and access keys; every action dispatch is exhaustively mapped; Windows CI has a native UI Automation smoke test for names, types, focus, bounds, long/non-ASCII input, dirty state, and close confirmation. Configuration replacement and initial creation are serialized by a named Windows mutex across processes in the current session. Thread regressions cover simultaneous replacement and first creation; a Windows-only integration test releases two independent processes against the same non-ASCII path and rejects partial or leftover staging state. Real visual/screen-reader/mixed-DPI evidence, first Windows results for the independent-process test, mutex timeout/abandonment, and external-state button/rollback automation remain open. See [the detailed accessibility contract](SETTINGS_ACCESSIBILITY.md).
