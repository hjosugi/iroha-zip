...
Progress (2026-08-15): the active workflow intentionally publishes unsigned Windows x64 and native ARM64 binaries with exact tag/version/current-`main` gates, two backend-free ZIPs, six individually downloadable PE executables, SHA-256 inventories, and GitHub workflow attestations. Publication creates an explicit draft, verifies all 11 remote assets by exact name/length/SHA-256 before publishing, and requires immutable/latest readback afterward. Future publication runs also allow only bounded propagation time before requiring the GitHub release attestation and `gh release verify-asset` success for every local asset; deterministic cross-platform PowerShell tests cover delayed availability, all-asset success, asset mismatch, exhaustion, incomplete inventory, and duplicate names. The unsigned [v0.6.2 release](https://github.com/hjosugi/iroha-zip/releases/tag/v0.6.2) passed the preceding complete publication path in [Actions run 31877964740](https://github.com/hjosugi/iroha-zip/actions/runs/31877964740), after [dry run 31877701103](https://github.com/hjosugi/iroha-zip/actions/runs/31877701103) passed without publication; [tag CI run 31877964744](https://github.com/hjosugi/iroha-zip/actions/runs/31877964744) also passed on the exact source commit. All public assets were independently downloaded; API digests and byte lengths, eight checksum subjects, two sidecars, x64/ARM64 PE identities inside and outside both ZIPs, empty Authenticode Certificate Tables, bilingual package content and source-document equality, backend non-inclusion, annotated tag object `d9fb686b830e24798c822b2d00e1a4d8bdbc8f37`, exact commit `4c53bb0b2666486c541b47606e54cf1345f672c3`, and nine hosted-runner-only tag-ref workflow attestations all matched. The separate GitHub release attestation bound that tag object to all 11 assets, and every downloaded asset passed `gh release verify-asset`. The fixed metadata, asset hashes, expiring workflow-artifact digests, and independent results are preserved in the [v0.6.2 release snapshot](https://github.com/hjosugi/iroha-zip/tree/main/evidence/releases/v0.6.2) and enforced by an ordinary Rust test. The package and website disclose the absence of Authenticode and provide Japanese/English verification guidance. The local build/sign/package split and strict WinVerifyTrust publisher/EKU/timestamp verifier remain available for a future owner-configured signing identity. Authenticode custody, the first signed run, and independent review remain open; no current release is claimed signed. See [the release contract](RELEASE_VERIFICATION.md).
