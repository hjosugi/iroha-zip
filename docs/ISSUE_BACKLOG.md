...
Progress (2026-08-14): native GitHub `windows-11-arm` CI now requires OS/process architecture `Arm64` and Rust host `aarch64-pc-windows-msvc`, runs all-target tests and Clippy, builds all three release executables, requires PE machine `0xAA64`, and executes the schema-v4 normal-AppContainer isolation contract. Downloaded evidence from [Actions run 31768309835](https://github.com/hjosugi/iroha-zip/actions/runs/31768309835) records zero capabilities, network/timeout/memory denial, abnormal-exit and corrupt-loader handling, process-temp and staging-DACL behavior, and seven successful profile/root cleanups. It also explicitly records `backendArchiveMatrixExecuted: false` and `releaseAssetPublished: false`. A supported CLANGARM64 backend evidence/export path, full archive/corpus/shell/settings/LPAC matrix, architecture-separated release inventory, and ARM64 assets remain open; see [the ARM64 boundary](ARM64.md).
