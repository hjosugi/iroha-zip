...
Progress (2026-08-14): v0.4.0 keeps each input archive/source-file handle through validation, hashing, and copy; verifies identity, timestamps, length, and SHA-256; blocks write/delete sharing on Windows; uses `O_NOFOLLOW` on Unix; and compares deterministic path/type/content fingerprints before and after source-tree copy. Windows tree membership comes from bounded `GetFileInformationByHandleEx` enumeration on reparse-aware, rename/delete-blocking directory handles. Creation rechecks staged source and generated PAX fingerprints, snapshots the archive, re-extracts it in a second sandbox, compares the full path/type/length/content tree, and publishes from a retained handle with create-new semantics. Windows stages source outside Package profile storage and grants the ephemeral Package SID inheritable read/execute only. The trusted parent emits a bounded deterministic PAX archive; a write/delete-nonshared handle is retained while bsdtar reads only the fixed sandbox-local `@source.pax.tar` name. This replaces the `@-` route that reproducibly crashed the MSYS2 UCRT64 backend in AppContainer without exposing the normal source tree. Fake-backend regressions cover success, mutation, unsafe members, mismatches, empty roots, and budgets. Native child open relative to the parent handle, a real-Windows reparse race stress test, and a passing Actions result for the new fixed-name path remain open.
