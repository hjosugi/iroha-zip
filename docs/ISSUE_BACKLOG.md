...
Progress (2026-08-14): the active workflow intentionally publishes unsigned Windows x64 binaries with exact tag/version/current-`main` gates, a complete ZIP, three individually downloadable PE executables, SHA-256 inventories, and GitHub artifact attestations. It refuses to overwrite an existing release and reads back the exact stable six-asset inventory. The package and website disclose the absence of Authenticode and provide Japanese/English verification guidance. The local build/sign/package split and strict WinVerifyTrust publisher/EKU/timestamp verifier remain available for a future owner-configured signing identity. Authenticode custody, immutable-release policy, the first signed run, and independent review remain open; no current release is claimed signed. See [the release contract](RELEASE_VERIFICATION.md).
...
Progress (2026-08-14): v0.4.0 keeps each input archive/source-file handle through validation, hashing, and copy; verifies identity, timestamps, length, and SHA-256; blocks write/delete sharing on Windows; uses `O_NOFOLLOW` on Unix; and compares deterministic path/type/content fingerprints before and after source-tree copy. Windows tree membership comes from bounded `GetFileInformationByHandleEx` enumeration on reparse-aware, rename/delete-blocking directory handles. For creation, the trusted parent emits and handle-pins a bounded deterministic PAX snapshot, removes the normal staging copy before child launch, and lets one monitored AppContainer pass materialize the PAX. The complete materialized tree must match the original fingerprint; the parent then deletes and recopies the verified backend, removes the PAX, DACL-seals the materialized tree read-only to the Package SID, and lets the fresh backend archive only relative `.`. This avoids the reproducible MSYS2 UCRT64 AppContainer access violation in `@archive` conversion without returning to the normal source path. The result is re-extracted in a second sandbox, compared by full path/type/length/content, and published from a retained handle with create-new semantics. Fake-backend regressions and a local libarchive 3.8.9 ZIP/7z/TAR/TAR.GZ/empty/Japanese-name round trip cover the new path. Native child open relative to the parent handle, a real-Windows reparse race stress test, and a passing Actions result for the two-pass path remain open.
